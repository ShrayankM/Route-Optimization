...
# GET NEIGHBORS
def get_neigh_cost(i, j, lt_ih, lt_jh):
    arr = []
    #(1) col - 1, row - 1
    if (j - 1 >= 0) and (i - 1 >= 0):
    ...
    #(9) col - 2, row - 1
    if (j - 2 < lt_jh) and (i - 1 < lt_ih):

        if (acc_cost[i - 1][j - 2] > c_anist_cost(i, j, i - 1, j - 2)):
            parent[i - 1][j - 2] = i, j
            acc_cost[i - 1][j - 2] = c_anist_cost(i, j, i - 1, j - 2)
        arr.append([acc_cost[i - 1][j - 2], [i - 1, j - 2], [i, j]])
    else:
        arr.append(math.inf)
    
    #(10) col - 1, row - 2
    if (j - 1 < lt_jh) and (i - 2 < lt_ih):

        if (acc_cost[i - 2][j - 1] > c_anist_cost(i, j, i - 2, j - 1)):
            parent[i - 2][j - 1] = i, j
            acc_cost[i - 2][j - 1] = c_anist_cost(i, j, i - 2, j - 1)
        arr.append([acc_cost[i - 2][j - 1], [i - 2, j - 1], [i, j]])
    else:
        arr.append(math.inf)
...
# DUMMY PIXELS
start_pixel, end_pixel = [8414, 1529], [8356, 1694]
...
while (pr[0] != -1) and (pr[1] != -1):
    path.append(pr)
    with open('routes-shape/dijk-route-knt/path.txt', 'a') as f:
        f.write(str(pr) + '\n')

    pr = parent[pr[0]][pr[1]]

path_list = []
with open('routes-shape/dijk-route-knt/path.txt', 'r') as f:
    for point in f:
...
w = shapefile.Writer('routes-shape/dijk-route-knt/shapefiles/test/multipoint')
w.field('name', 'C')
