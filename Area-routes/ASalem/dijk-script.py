...
# Study Area LOAD
area_cover = 'Area-routes/ASalem/mask.tif'
area = rasterio.open(area_cover, count = 1)
area = np.array(area.read(1))

# DEM LOAD
area_dem = 'Area-routes/ASalem/dem_clipped.tif'
...
upscale_factor = 3
with rasterio.open('Area-routes/ASalem/dem_clipped.tif') as dataset:
...
file = ogr.Open("Area-routes/ASalem/source/source.shp")
...
file = ogr.Open("Area-routes/ASalem/destination/destination.shp")
...
# GET Raster Extent
path = 'Area-routes/ASalem/raster.tif' 
...
while (pr[0] != -1) and (pr[1] != -1):
    path.append(pr)
    with open('Area-routes/ASalem/routes-shape/dijk-route/path.txt', 'a') as f:
        f.write(str(pr) + '\n')

    pr = parent[pr[0]][pr[1]]

path_list = []
with open('Area-routes/ASalem/routes-shape/dijk-route/path.txt', 'r') as f:
    for point in f:
...
w = shapefile.Writer('ASalem/routes-shape/dijk-route/shapefiles/test/multipoint')
w.field('name', 'C')
