...
while (pr[0] != -1) and (pr[1] != -1):
    path.append(pr)
    with open('routes-shape/dijk-route/path.txt', 'a') as f:
        f.write(str(pr) + '\n')

    pr = parent[pr[0]][pr[1]]

path_list = []
with open('routes-shape/dijk-route/path.txt', 'r') as f:
    for point in f:
...
w = shapefile.Writer('routes-shape/dijk-route/shapefiles/test/multipoint')
w.field('name', 'C')
