...
# DUMMY PIXELS
# start_pixel, end_pixel = [667, 174], [746, 117]
...
for f in range(1, 6):
    ...
    while (condition_check(s_pixel, e_pixel)):
        ...
        neighbours_cost = get_neigh_cost(i, j, FLAG, lt_ih, lt_jh)
    ...
    os.mkdir(f'routes-shape-DEM/{route_paths[FLAG]}/')
    ...
    while (pr[0] != -1) and (pr[1] != -1):
        path.append(pr)
        # path.append('-')
        with open(f'routes-shape-DEM/{route_paths[FLAG]}/path.txt', 'a') as f:
            f.write(str(pr) + '\n')
    ...
    path_list = []
    with open(f'routes-shape-DEM/{route_paths[FLAG]}/path.txt', 'r') as f:
        for point in f:
    ...
    # import shapefile
    w = shapefile.Writer(f'routes-shape-DEM/{route_paths[FLAG]}/shapefiles/test/multipoint')
    w.field('name', 'C')
